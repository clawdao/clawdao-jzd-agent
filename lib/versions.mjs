...
export class VersionManager {
  ...
  async create(body) {
    ...
    // ★ releaseNotes 入参语义：
    //   - string：可能是 markdown 或 HTML（自动转 HTML）
    //   - object {zh:[], en:[]}：官方 schema（必须 zh 是数组）
    // ★★★ server PUT/POST 只接受 releaseNotes 为 string ★★★
    const releaseNotesInput = body.releaseNotes;
    let releaseNotesStr;
    if (typeof releaseNotesInput === 'string') {
      releaseNotesStr = releaseNotesInput.trim().startsWith('<') ? releaseNotesInput : mdToHtml(releaseNotesInput);
    } else if (releaseNotesInput && typeof releaseNotesInput === 'object' && Array.isArray(releaseNotesInput.zh)) {
      // 是 {zh: [html1, html2]} 形式 → 合并为单一 HTML 字符串
      releaseNotesStr = releaseNotesInput.zh.join('\n');
    } else {
      releaseNotesStr = '';
    }
    const payload = {
  ...
  async update(versionId, body) {
    // ★ releaseNotes 同样：string 直接传，object 合并成 string
    if (body.releaseNotes && typeof body.releaseNotes === 'object' && Array.isArray(body.releaseNotes.zh)) {
      body = { ...body, releaseNotes: body.releaseNotes.zh.join('\n') };
    } else if (typeof body.releaseNotes === 'string' && !body.releaseNotes.trim().startsWith('<')) {
      body = { ...body, releaseNotes: mdToHtml(body.releaseNotes) };
    }
    return this.#client.request({
      method: 'PUT',
      path: `/api/v1/product/versions/${versionId}`,
