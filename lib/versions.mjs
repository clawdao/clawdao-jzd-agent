...
export class VersionManager {
   ...
   * @param {object} params
   *   productKey  {string}  产品 slug（如 'clawdao'）
   *   version     {string}  semver，如 '1.0.27'
   *   prefix      {string}  可选 S3 前缀
   *   dryRun      {boolean} 仅预览
   */
  async syncFromS3({ productKey, version, prefix, dryRun = false } = {}) {
    if (!productKey) {
      return { ok: false, error: { kind: 'validation', message: 'productKey 必填（如 clawdao）' } };
    }
    if (!version) {
      return { ok: false, error: { kind: 'validation', message: 'version 必填（如 1.0.27）' } };
    }
    return this.#client.request({
      method: 'POST',
