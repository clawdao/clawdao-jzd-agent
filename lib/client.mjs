...
export class JzdClient {
  ...
  constructor(config = {}) {
    ...
    this.#config = {
      ...
      // ★ 平台级 token（product/versions 等平台级操作）
      platformToken: trimToUndefined(config.platformToken || process.env.DDN_HUB_PLATFORM_TOKEN) || null,
  ...
  getRuntimeSnapshot() {
    ...
    return {
      ...
      hasPlatformToken: Boolean(c.platformToken),
      hasDaoId: Boolean(c.daoId),
      daoId: c.daoId,
    };
  }

  async request({ method, path, query, body, authRequired = false, daoRequired = false, daoId, platformToken = false }) {
    if (!this.#config.baseUrlValid) {
      return this.#fail('config_error', `DDN_HUB_BASE_URL 无效: ${this.#config.baseUrlError}`);
    }

    // ★ platformToken = true 时使用 DDN_HUB_PLATFORM_TOKEN（平台级权限）
    const authToken = platformToken ? (this.#config.platformToken || this.#config.authToken) : this.#config.authToken;
    if (authRequired && !authToken) {
      return this.#fail('config_error', platformToken
        ? '需要 DDN_HUB_PLATFORM_TOKEN 或 DDN_HUB_AUTH_TOKEN 才能调用此平台级工具'
        : '需要 DDN_HUB_AUTH_TOKEN 才能调用此工具');
    }
    ...
    // ★ 如果提供了 authToken 覆盖（如 platformToken），使用它
    const finalAuthToken = authToken || this.#config.authToken;
    if (authRequired && !finalAuthToken) {
      return this.#fail('config_error', '需要鉴权 token（DDN_HUB_AUTH_TOKEN 或 DDN_HUB_PLATFORM_TOKEN）');
    }
